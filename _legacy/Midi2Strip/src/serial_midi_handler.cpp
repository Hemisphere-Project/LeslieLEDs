...
#define CONNECTION_TIMEOUT_MS 5000  // Consider disconnected after 5s without data
...
void SerialMIDIHandler::setLEDController(LEDController* controller) {
...
void SerialMIDIHandler::handleControlChange(uint8_t channel, uint8_t cc, uint8_t value) {
    ...
    // Route CCs to appropriate handlers (same logic as USB MIDI handler)
    if (cc >= 1 && cc <= 19) {
        // Global controls
        ledController->handleGlobalCC(cc, value);
    } else if (cc >= 20 && cc <= 29) {
        // Color A controls
        ledController->handleColorCC(0, cc, value);
    } else if (cc >= 30 && cc <= 39) {
        // Color B controls
        ledController->handleColorCC(1, cc, value);
...
void SerialMIDIHandler::handleNoteOn(uint8_t channel, uint8_t note, uint8_t velocity) {
    if (ledController) {
        ledController->handleNoteOn(note, velocity);
    }
...
void SerialMIDIHandler::handleNoteOff(uint8_t channel, uint8_t note, uint8_t velocity) {
    if (ledController) {
        ledController->handleNoteOff(note);
    }
